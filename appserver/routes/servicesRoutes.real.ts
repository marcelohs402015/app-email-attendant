...
export function createServicesRoutesReal(db: Database): Router {
  ...
  router.post('/services', async (req: Request, res: Response) => {
    try {
      const { name, description, category, price = 0, unit = 'hour', estimatedTime, materials = [], active = true } = req.body;
      ...
      const result = await db.query(
        'INSERT INTO services (id, name, description, category, price, unit, estimated_time, materials, active) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *',
        [id, name, description, category, parseFloat(price), unit, estimatedTime, JSON.stringify(materials), active]
      );
  ...
  router.put('/services/:id', async (req: Request, res: Response) => {
    try {
      ...
      if (estimatedTime !== undefined) { fields.push(`estimated_time = $${idx++}`); values.push(estimatedTime); }
