...
export function createCategoryRoutesReal(db: Database): Router {
  ...
  router.get('/categories', async (_req: Request, res: Response) => {
    try {
      const result = await db.query<any>(
        'SELECT id, name, description, keywords, patterns, domains, color, active, created_at, updated_at FROM categories ORDER BY name ASC'
      );
  ...
  router.post('/categories', async (req: Request, res: Response) => {
    try {
      const { name, description, keywords = [], patterns = [], domains = [], color = '#3B82F6' } = req.body || {};
      if (!name || !description) {
        return res.status(400).json({ success: false, error: 'Name and description are required' });
      }
      const insert = await db.query<any>(
        'INSERT INTO categories (name, description, keywords, patterns, domains, color) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id,name,description,keywords,patterns,domains,color,active,created_at,updated_at',
        [name, description, JSON.stringify(keywords), JSON.stringify(patterns), JSON.stringify(domains), color]
      );
  ...
  router.put('/categories/:id', async (req: Request, res: Response) => {
    try {
      ...
      if (patterns !== undefined) { fields.push(`patterns = $${idx++}`); values.push(JSON.stringify(patterns)); }
      if (domains !== undefined) { fields.push(`domains = $${idx++}`); values.push(JSON.stringify(domains)); }
      ...
      const update = await db.query<any>(
        `UPDATE categories SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = $${idx} RETURNING id,name,description,keywords,patterns,domains,color,active,created_at,updated_at`,
        values
      );
