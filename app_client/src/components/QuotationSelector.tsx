...
export default function QuotationSelector({ 
  ...
  const getStatusLabel = (status: string) => {
    return t(`quotations.status.${status}`);
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[80vh] overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-gray-900">{t('quotations.selectQuotation')}</h3>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto max-h-[60vh]">
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">{t('quotations.noQuotationsFound')}</p>
              <p className="text-sm text-gray-400 mt-2">
                {t('quotations.createFirstHint')}
              </p>
            </div>
          ) : (
              ...
              <div className="mb-4">
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="font-medium text-gray-700">{t('quotations.noAttachment')}</span>
                  </div>
                </button>
              </div>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        <h4 className="font-medium text-gray-900">
                          {t('quotations.quotationFor')} {quotation.clientName}
                        </h4>
                      ...
                      <div className="text-sm text-gray-600 space-y-1">
                        <p><strong>{t('quotations.client')}:</strong> {quotation.clientEmail}</p>
                        ...
                        <p><strong>{t('quotations.created')}:</strong> {
                          format(parseISO(quotation.createdAt), 'dd/MM/yyyy', { locale: enUS })
                        }</p>
                        ...
                        {quotation.notes && (
                          <p><strong>{t('quotations.notes')}:</strong> {quotation.notes}</p>
                        )}
                      </div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        ...
                        {quotation.items.length > 3 && (
                          <span className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded">
                            +{quotation.items.length - 3} {t('quotations.more')}
                          </span>
                        )}
                      </div>
                    </div>
