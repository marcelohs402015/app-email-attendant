...
Equipe Técnica`,
        ...
        for (const template of templates) {
            try {
                await this.query(`INSERT INTO email_templates (name, subject, body, category, variables) 
           VALUES ($1, $2, $3, $4, $5) 
           ON CONFLICT DO NOTHING`, [template.name, template.subject, template.body, template.category, JSON.stringify(template.variables)]);
            }
            catch (error) {
                this.logger.warn(`Failed to insert template ${template.name}:`, error.message);
            }
        }
